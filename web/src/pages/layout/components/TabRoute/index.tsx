...
const TabRoute = function(clickChangeMaximize: React.MouseEventHandler<HTMLElement> | undefined, showMaximize: any) {
  ...
  return (
    <Tabs id={tabsDivId.current} activeKey={location.pathname} onChange={(key) => selectTab(key)} tabBarExtraContent={operations} tabBarStyle={{ background: '#fff' }} tabPosition='top' animated tabBarGutter={0} hideAdd type='editable-card' onEdit={(targetKey) => closeTab(targetKey)}>
