...
type SysOperationRecordModel struct { 
    ...
    IP            string       `form:"ip" gorm:"column:ip" json:"ip"`                                // 请求ip
    Method        string       `form:"method" gorm:"column:method" json:"method"`                    // 请求方法
    Path          string       `form:"path" gorm:"column:path" json:"path"`                          // 请求路径
    Status        int          `form:"status" gorm:"column:status" json:"status"`                    // 请求状态
    Latency       int          `form:"latency" gorm:"column:latency" json:"latency"`                 // 延迟
    Agent         string       `form:"agent" gorm:"column:agent" json:"agent"`                       // 代理
    ErrorMessage  string       `form:"error_message" gorm:"column:error_message" json:"errorMessage"` // 错误信息
    RequestBody   string       `form:"request_body" gorm:"column:request_body" json:"requestBody"`   // 请求Body
    ResponseBody  string       `form:"response_body" gorm:"column:response_body" json:"responseBody"` // 响应Body
    UserID        int64        `form:"user_id" gorm:"column:user_id" json:"userID"`                  // 用户ID
    Level         int          `form:"level" gorm:"column:level" json:"level"`                       // 1:增删改查，2：增删改，3：删改
...
var SysOperationRecordColumn = struct {
    ID                   string
	IP                   string
	Method               string
	Path                 string
